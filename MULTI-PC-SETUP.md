...
## Managing User Accounts Over Time

### Deactivating a User (Temporary Block)

If a staff member leaves or takes a break, deactivate them so they cannot log in — all their data stays fully intact.

1. Admin → **Users** tab → find the user → click **Deactivate**
2. Their account is blocked immediately; they see "Account deactivated" if they try to log in
3. **Their data is fully preserved:** registrations they submitted, session notes, and mentor assignments remain unchanged
4. **Mentor's teens:** If the deactivated user is a Mentor, their assigned teens remain in the system — reassign in the **Mentors** tab
5. To re-enable later: click **Activate** next to their name

### Deleting a User (Soft Archive)

Once a user is deactivated, an Admin can permanently remove them from the active user list.

1. Admin → **Users** tab → deactivate the user first (if not already done)
2. Click the red **Delete** button that appears next to inactive users
3. The user moves to the **Deleted Users** archive tab — **nothing is erased from the database**
4. All their registrations, session notes, and records remain intact

> ⚠️ An Admin cannot delete their own account.

### Restoring a Deleted User

If a deleted user returns or was removed by mistake:

1. Admin → **Users** tab → click **Deleted Users** tab
2. Find the user → click **↩ Restore & Reactivate**
3. Their account is reactivated immediately — same role, same credentials

### Deactivated Data-Entry User — Correction Handling

If admin **denies** a submission that was created by a deactivated or deleted data-entry person:

- The deny dialog shows a warning: *"[Name] is deactivated and cannot receive this correction"*
- Admin chooses one of:
  - **Reassign to another active data-entry user** — pick someone from the dropdown to handle the resubmission

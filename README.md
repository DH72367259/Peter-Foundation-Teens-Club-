...
4. Click **Create User**
5. To **deactivate** someone (block login): click **Deactivate** next to their name
   - All their data is fully preserved — registrations, session notes, mentor assignments
   - Mentor's assigned teens remain listed and can be reassigned in the Mentors tab
6. To **delete** a user permanently from the active list: first **Deactivate** them, then click **Delete**
   - Deleted users move to the **Deleted Users** archive tab — nothing is erased from the database
   - Click **Deleted Users** tab to see the archive
7. To **restore** a deleted user: click **Deleted Users** tab → click **↩ Restore & Reactivate**
   - They can log in again immediately with the same credentials and role
8. To **reset a password**: click **Reset Pwd** next to any active user

> ⚠️ Admin cannot deactivate or delete their own account.
...
**When the original data-entry person is deactivated/deleted:**
If a pending submission was created by a deactivated or deleted staff member, the **❌ Deny** dialog shows a warning and two options:
- **Reassign to another data-entry user** — pick an active DATA ENTRY person from the dropdown; they will receive the correction request and handle the resubmission
- **I'll edit it myself** — admin denies the record and is taken directly to the edit form
...
| **Users (Active)** | Admin | Create user accounts, activate/deactivate, reset passwords, delete (soft) |
| **Users (Deleted)** | Admin | Archive of deleted users; restore and reactivate any deleted user |
| **Age-Out Management** | Admin | Review teens who have aged out (18+); archive or extend stay |

---

### User Lifecycle

Users can move through the following states:

| State | Can log in? | Data preserved? | Can be reversed? |
|---|:---:|:---:|:---:|
| **Active** | ✅ Yes | ✅ Yes | — |
| **Deactivated** | ❌ No | ✅ Yes | ✅ Yes — reactivate with Activate button |
| **Deleted** | ❌ No | ✅ Yes | ✅ Yes — restore from Deleted Users tab |

**Important rules:**
- A user must be **deactivated first** before they can be deleted
- Admin cannot deactivate or delete their own account
- When a mentor is deactivated: their assigned teens remain in the system and can be reassigned
- When a data-entry user is deactivated: their submitted records still appear for admin approval
- If admin tries to deny a submission from a deactivated/deleted user, they are prompted to reassign it to another active data-entry person (or edit it themselves)
- All session notes, registrations, and mentor history remain intact regardless of user status

---

### Age-Out Management

Teens turn 18 and eventually age out of the programme. The system tracks this automatically.

**How it works:**
- Every teen's age is tracked from their Date of Birth
- Teens who have turned **18 or older** appear in the **Age-Out Management** page (Admin-only tab on Home)
- Admin reviews each aged-out teen and chooses:
  - **Archive** — marks the teen as inactive (hidden from search results, all data preserved)
  - **Extend Stay** — keeps the teen active with a note

**To access Age-Out Management:**
1. Log in as Admin → **Home** dashboard
2. Click the **Age-Out Management** tab (count badge shown if any teens have aged out)
3. Review the list → click **Archive** or **Extend Stay** for each teen
